# Instructions pour ajouter les images

## 1. Photo principale du bus

Placez l'image du bus jaune "Mohammed V School Bus" dans le dossier `public/` avec le nom :
- `bus-main.jpg` (recommandé) ou `bus-main.png`

Cette image sera utilisée comme :
- Image principale sur la page d'accueil
- Favicon du site (icône dans l'onglet du navigateur)

**Note :** L'image `bus-main.jpg` sera utilisée à la fois pour la page d'accueil et comme favicon.

## 2. Fichier audio du claxon

Placez un fichier audio de claxon de bus scolaire dans le dossier `public/` avec le nom :
- `school_bus_horn_short.mp3`

Vous pouvez télécharger un fichier audio gratuit depuis :
- https://freesound.org (rechercher "school bus horn")
- https://zapsplat.com (rechercher "school bus horn")

Le fichier doit être en format MP3 et d'une durée d'environ 1-2 secondes.

## Note

Une fois les fichiers ajoutés, le code les utilisera automatiquement.
