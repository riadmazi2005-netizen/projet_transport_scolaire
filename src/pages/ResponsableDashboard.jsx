...
// Composant pour les filtres de la liste des élèves (déplacé ici pour éviter les re-montages)
const ElevesListFilters = ({
      ...
      {dateFilter === 'custom' && (
        <div className="relative">
          ...
          <Input
            ...
            className="bg-white/90 border-0 rounded-xl pl-10 focus:ring-purple-500 focus:border-purple-500"
          />
        </div>
      )}
    </div>
...
export default function ResponsableDashboard() {
  ...
  const [elevesPresences, setElevesPresences] = useState([]);
    ...
    if (activeTab === 'eleves' && bus && responsable) {
      const loadPresencesForDate = async () => {
        try {
          ...
          console.log('✅ Présences report filtrées pour date', effectiveDate, ':', filteredPresences.length, 'présences');
          setElevesPresences(filteredPresences);
        } catch (err) {
          console.error('❌ Erreur chargement présences report:', err);
          setElevesPresences([]);
        }
      };
  ...
  // Recharger les présences pour le marquage quand la date change
  useEffect(() => {
    if (activeTab === 'presence' && bus && responsable) {
      const loadMarkingPresences = async () => {
        try {
          const responsableId = responsable?.type_id || responsable?.id;
          const presencesResponse = await presencesAPI.getByDate(selectedDate, bus.id);
          const presencesData = presencesResponse?.data || presencesResponse || [];
          const filteredPresences = presencesData.filter(p =>
          ...
          console.warn('Erreur chargement présences marquage:', err);
          setPresences([]);
        }
      };
      loadMarkingPresences();
    }
  }, [selectedDate, activeTab, bus?.id, responsable?.type_id, responsable?.id]);
    ...
    try {
      ...
      if (existingPresence) {
        ...
        setPresences(prev => prev.map(p =>
          p.id === (existingPresence.id || existingPresence.presence_id)
            ? { ...p, ...presenceData }
            : p
        ));
      } else {
          ...
          {activeTab === 'eleves' && (
            <div className="bg-white rounded-3xl shadow-xl overflow-hidden">
              ...
              {(() => {
                ...
                const elevesMatin = filteredElevesList.filter(eleve => {
                  const presence = elevesPresences.find(p => p.eleve_id === eleve.id && normalizeDate(p.date) === normalizedEffectiveDate);
                  if (!presence) return false;
                  return presence.present_matin === true || presence.present_matin === 1 || presence.present_matin === '1';
                });

                const elevesSoir = filteredElevesList.filter(eleve => {
                  const presence = elevesPresences.find(p => p.eleve_id === eleve.id && normalizeDate(p.date) === normalizedEffectiveDate);
                  if (!presence) return false;
                  return presence.present_soir === true || presence.present_soir === 1 || presence.present_soir === '1';
                });
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-8">
                        <div className="text-center">
                          <p className="text-2xl font-bold text-gray-700">{filteredElevesList.length}</p>
                          <p className="text-[10px] uppercase font-bold text-gray-400">Total</p>
                        </div>
                        <div className="w-px h-8 bg-gray-200" />
                        <div className="flex gap-4">
                          <div className="text-center">
                            <p className="text-xl font-bold text-green-600">{elevesMatin.length}</p>
                            <p className="text-[10px] uppercase font-bold text-gray-400">Présents Matin</p>
                          </div>
                          <div className="text-center">
                            <p className="text-xl font-bold text-green-700">{elevesSoir.length}</p>
                            <p className="text-[10px] uppercase font-bold text-gray-400">Présents Soir</p>
                          </div>
                        </div>
                      </div>
              ...
              <div className="divide-y divide-gray-100 max-h-[500px] overflow-y-auto">
                {filteredElevesList.map((eleve) => {
                  ...
                  const elevePresence = elevesPresences.find(p => {
                  ...
                  return (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="p-4 hover:bg-purple-50/5 transition-colors"
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <div className="flex flex-col items-center gap-1">
                            <span className="text-[10px] uppercase font-bold text-gray-400">Matin</span>
                            <div className={`px-3 py-1.5 rounded-lg text-xs font-bold border flex items-center gap-2 ${!elevePresence || elevePresence.present_matin === null || elevePresence.present_matin === undefined
                              ? 'bg-gray-50 text-gray-400 border-gray-200'
                              : (elevePresence.present_matin === true || elevePresence.present_matin === 1 || elevePresence.present_matin === '1')
                                ? 'bg-green-50 text-green-600 border-green-200'
                                : 'bg-red-50 text-red-600 border-red-200'
                              }`}>
                              {(!elevePresence || elevePresence.present_matin === null || elevePresence.present_matin === undefined) ? (
                          ...
                          {/* Badge Soir */}
                          <div className="flex flex-col items-center gap-1">
                            <span className="text-[10px] uppercase font-bold text-gray-400">Soir</span>
                            <div className={`px-3 py-1.5 rounded-lg text-xs font-bold border flex items-center gap-2 ${!elevePresence || elevePresence.present_soir === null || elevePresence.present_soir === undefined
                              ? 'bg-gray-50 text-gray-400 border-gray-200'
                              : (elevePresence.present_soir === true || elevePresence.present_soir === 1 || elevePresence.present_soir === '1')
                                ? 'bg-green-50 text-green-600 border-green-200'
                                : 'bg-red-50 text-red-600 border-red-200'
                              }`}>
                              {(!elevePresence || elevePresence.present_soir === null || elevePresence.present_soir === undefined) ? (
                                <Clock className="w-3 h-3" />
                              ) : (elevePresence.present_soir === true || elevePresence.present_soir === 1 || elevePresence.present_soir === '1') ? (
                                <Check className="w-3 h-3" strokeWidth={3} />
                              ) : (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex justify-between py-2">
                        ...
                        <span className={`px-3 py-1 rounded-full text-sm font-medium ${bus.statut === 'Actif' ? 'bg-purple-100 text-purple-700' : 'bg-red-100 text-red-700'
                          }`}>
                          {bus.statut}
                        </span>
                      </div>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div className="ml-auto">
                        <div className="flex items-center gap-2">
                          ...
                          <span className={`font-bold ${chauffeur.nombre_accidents >= 3 ? 'text-red-500' : 'text-purple-500'
                            }`}>
                              ...
                              <div className="flex items-center gap-3">
                                ...
                                <div>
                                  <div className="flex items-center gap-2 mb-1">
                                    ...
                                    {accident.statut && (
                                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${accident.statut === 'Traité' ? 'bg-green-100 text-green-700' :
      ...
      <AnimatePresence>
        {toastMessage && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -50 }}
            className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] ${toastType === 'success'
              ? 'bg-green-500'
              : toastType === 'error'
                ? 'bg-red-500'
                : 'bg-purple-500'
